...
$(document).ready(function () {
    ...
    $("#font-size").on("change", function () {
        ...
        $("#grid .cell.selected").css("font-size",parseInt(fontSize) );
        let cellElem = $("#grid .cell.selected");
        let cellObject = getcell(cellElem);
        cellObject.fontSize = fontSize;

//*************************** *done by me below 3 lines

let { rowId } = getrc(lcell);//lcell contain cell address of last cell selected
        let ht = $(lcell).height();
        $($("#left-col .cell")[rowId]).height(ht);
    ...
    $("#grid .cell").on("blur", function () {
        ...
        lsc = this;
    //since  this lsc will now have reference of newly selected cell but blur is called on
    //previous cell so we must know where blur called so ass to update formula dor that cell
